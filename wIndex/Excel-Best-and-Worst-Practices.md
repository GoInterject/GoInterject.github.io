...
In most reports, selecting the entire row for Column Definitions in [ReportRange()](/wFunctions/ReportRange.html) is fast and easy. However, when using larger, more complex reports, it is better only to include the columns needed for the report.
...
The next example shows the same report, but only the impacted columns are included in the column definitions and target range. This is more efficient. However, keep in mind the argument for [ReportRange()](/wFunctions/ReportRange.html), UseEntireRow is set to False. By setting to True, it converts the ranges to entire rows.
...
The old Microsoft Excel type .xls does not compress the data in the report using saving so this type of file will be much larger. Always use the xlsx or xlsm file type for your reports.
